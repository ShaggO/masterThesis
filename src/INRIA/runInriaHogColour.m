...
method = methodStruct( ...
    'window',{'type','square','scales',2^(1/3).^(0:6),'spacing',10,'windowSize',windowSize}, ...
    'hog',{'numorientations',9,'colour','none'}, ...
    0,{'rx-'});

data = inriaData;
diaryFile = ['inriaParametersHogColour_' strrep(datestr(now),':','-') '.out'];
diary(diaryFile)
disp('Optimization of svm for HOG (colour) started.');
diary off;
...
clear data;
save('results/optimize/inriaParametersHogColour');
