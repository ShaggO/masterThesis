...
for split = 1:6
    ...
    for i = 1:iters
        ...
        diary off
        method = enumOptimizeParameter(setNumTrain,method,diaryFile,logger,'gridType',gridTypes,'gridSize',gridSizes,'cellFilter',cellFilters);
        method = zoomOptimizeParameter(setNumTrain,method,diaryFile,logger,'gridRadius', ...
            (5:2.5:20)',(-1:0.5:1)');
        method = modifyDescriptor(method,'centerFilter','gaussian');
        method = zoomOptimizeParameter(setNumTrain,method,diaryFile,logger,'centerSigma', ...
            repmat((0.5:0.5:2)',[1 2]),repmat((-0.2:0.1:0.2)',[1 2]));
        method = enumOptimizeParameter(setNumTrain,method,diaryFile,logger,'centerFilter',{'gaussian','none'});
        method = zoomOptimizeParameter(setNumTrain,method,diaryFile,logger,'cellSigma', ...
            repmat((0.5:0.5:2)',[1 2]),repmat((-0.2:0.1:0.2)',[1 2]));
    end
    diary(diaryFile)
    disp([timestamp(startTime) ' Optimizing other parameters: ']);
    diary off
    method = zoomOptimizeParameter(setNumTrain,method,diaryFile,logger,'binSigma', ...
        (0.5:0.5:2)',(-0.2:0.1:0.2)');
    method = zoomOptimizeParameter(setNumTrain,method,diaryFile,logger,'binCount', ...
        (4:2:16)');
    method = zoomOptimizeParameter(setNumTrain,method,diaryFile,logger,'normSigma', ...
