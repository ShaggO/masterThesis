...
%% compute
matchROCAUC = [cat(1,go.testROCAUC),cat(1,si.testROCAUC),sift.matchROCAUC];
matchPRAUC = [cat(1,go.testPRAUC),cat(1,si.testPRAUC),sift.matchPRAUC];

pathTypes = 1:6;
[imNumKey,liNumKey,imNum,liNum,pathLabels] = dtuPaths('test');
plotROCAUC = cell(3,numel(imNum));
...
for p = pathTypes
    ...
    pIdx = idx2spil(:,2) == p;
    roc = reshape(matchROCAUC(pIdx,:),[numel(liNum{p}) numel(imNum{p}) numel(setNum) 3]);
    pr = reshape(matchPRAUC(pIdx,:),[numel(liNum{p}) numel(imNum{p}) numel(setNum) 3]);
    ...
    plotROCAUC(:,p) = mat2cell(roc, ones(1, 3));
    plotPRAUC(:,p) = mat2cell(pr, ones(1, 3));
end

displayDtuResults(plotROCAUC,plotPRAUC,pathTypes,'test',{{'-k'},{'-b'},{'-r'}},{'go','si','sift'}');
