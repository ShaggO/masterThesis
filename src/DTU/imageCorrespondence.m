function [matches, dimsM] = imageCorrespondence( ...
        ...
        for l = liNum
            for m = 1:numel(mFunc)
                ...
                if loaded && ismember('match',fieldnames(matchLoad))
                    ...
                    if numel(matchLoad.dims) > 1
                        matchLoad.dims = matchLoad.dims(m);
                        save(matchPath,'-struct','matchLoad');
                    else
                        dimsM(m) = matchLoad.dims;
                    end
                else
                    match.setNum = s;
                    match.imNum = i;
