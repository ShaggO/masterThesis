function [] = drawCellLayout(cellOffsets,centerType,centerSigma,cellType,cellSigma)
...
structArgs = interweave(fieldnames(S), ...
    repmat({zeros(prod(2*cellR+1),1,nCells,0)},[numel(fieldnames(S)) 1]));
Y = struct(structArgs{:});
...
for j = 1:numel(sigmaS)
    ...
    edgeFilter = all(c(:,1:2) + repmat(minOffset-cellR,[nJ 1]) >= 1,2) & ...
        all(c(:,2:-1:1) + repmat(maxOffset+cellR,[nJ 1]) <= ...
        repmat(sizeS, [nJ 1]),2);
    c = c(edgeFilter,:);
    Fout = F(idxJ,:);
    X = [X; Fout(edgeFilter,:)];
    ...
    if nJ > 0
        ...
        pCellIdx = sub2ind(sizeS,pCell(1,2,:,:),pCell(1,1,:,:));
        % compute relative indices for one cell
        relIdx = sub2ind(sizeS,cellMeshY,cellMeshX)-1;
...
Itotal = zeros(sizeS);
for i = 1:size(cellIdx,3)
    I = zeros(sizeS);
    I(cellIdx(:,:,i)) = W(:,:,i);
    Itotal = Itotal + I;
end
surf(Itotal,'FaceAlpha',0.8,'EdgeColor','none');
colormap jet
view([1 2 3])
axis([1 401 1 401 0 4*max(W(:))])
axis off
