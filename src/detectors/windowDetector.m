function F = windowDetector(Isize,type,scales,spacing,windowSize)
...
for i = 1:numel(scales)
    ri = spacing*scales(i)/2;
    windowSizei = windowSize*scales(i);
    switch type
        case 'square'
            x = 0:2*ri:(Isize(2)-windowSizei(2))/2;
            x = c(1) + [-x(end:-1:2) x];
            y = 0:2*ri:(Isize(1)-windowSizei(1))/2;
            y = c(2) + [-y(end:-1:2) y];
            Fi = [repmat(x,[numel(y) 1]) repmat(y',[1 numel(x)])];
            Fi = reshape(Fi,[],2);
        case 'triangle'
            x1 = 0:2*sqrt(3)*ri:(Isize(2)-windowSizei(2))/2;
            x1 = c(1) + [-x1(end:-1:2) x1];
            x2 = sqrt(3)*ri:2*sqrt(3)*ri:(Isize(2)-windowSizei(2))/2;
            x2 = c(1) + [-x2(end:-1:1) x2];
            y1 = 0:2*ri:(Isize(1)-windowSizei(1))/2;
            y1 = c(2) + [-y1(end:-1:2) y1];
            y2 = ri:2*ri:(Isize(1)-windowSizei(1))/2;
