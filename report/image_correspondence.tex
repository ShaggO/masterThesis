...
%
We will use the DTU dataset to evaluate our GO, SI, and GO+SI descriptors on the image correspondence problem. Our descriptors naturally use the interest point cell layouts for this application, described in \Cref{sec:cellApertureFunctionPoint}. Additionally we will evaluate SIFT in order to compare our results. As mentioned in \Cref{sec:opponentColourSpace}, the descriptors will be computed for opponent colour channels and concatenated, obtaining slightly better results. Each descriptor will be computed for the same interest points from a multi-scale DoG detector, described in \Cref{sec:multiscaleDoG}. This ensures that only the choice of descriptor can affect the result.
%
