...
%
The matching performance depends on the matching strategy. \todo{Rewrite introduction} \citet{mikolajczyk2005performance} described the following three matching strategies that are used when solving the image correspondence problem:
...
We have chosen to compare our descriptor performance against the SIFT descriptor since \citet{dahl2011finding} found the combination of DoG and SIFT to perform well. We are however using the opponent colour space for our descriptors, and since \citet{dahl2012interest} found Opponent SIFT (see \Cref{sec:opponentColourSpace}) to perform well against other SIFT variants, this is the descriptor we will test our performance against. Throughout this section we refer to Opponent SIFT as SIFT.\todo{Is this redundant information (Application section)}
