...
Simple \emph{thresholding} compares each descriptor in image $A$ with each descriptor in image $B$. Any two descriptors having a distance to one another below a threshold $t$ are classified as matches. This gives a one-to-many relation where each descriptor in $A$ can be matched to potentially all descriptors in $B$.
For a descriptor from $A$, \emph{Best-thresholding} finds the best matching descriptor from $B$. If the distance is below a threshold $t$, it is a match. Otherwise the descriptor from $A$ has no match. 
\emph{ratio-thresholding}
...
\begin{figure}
	...
	\caption{Overview of camera positions from \cite{aanaes2010recall}}
