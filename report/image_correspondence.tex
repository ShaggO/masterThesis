...
The DTU dataset is built to test the ability of computer vision systems to cope with viewpoint changes, scaling, light changes, and to a certain extent occlusions as well. Since all images are captured with the same camera tilt, no rotation of objects occurs and hence we refrain from pursuing rotational invariance.

NOTE: Compare with other datasets traditionally used: Good amount of viewpoints, not as natural images as other sets, no rotation. Well structured approach to changes in viewpoints and scale
