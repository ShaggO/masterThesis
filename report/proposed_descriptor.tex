...
From the blurred and downsampled images we apply derivative filters in order to compute value and magnitude images by their respective functions. In this case only $x$ and $y$-derivatives are needed to compute gradient orientation and magnitude. These images are shown in \Cref{fig:cellHistScaleSpacesV,fig:cellHistScaleSpacesM}. The magnitude image is then pre-processed by computing the pixel normalized magnitudes as shown in \Cref{fig:cellHistScaleSpacesMnorm} before being used as weights for the cell histograms. \Cref{fig:cellHistScaleSpacesBins} shows bin 1 (\Cref{fig:cellHistScaleSpacesBin01}) and 7 (\Cref{fig:cellHistScaleSpacesBin07}) of the 12 histogram bins with each pixel weighted by its pixel normalized magnitude. The cells are ignored in these two images. The images clearly show their corresponding parts of the beer can structures as well as some background noise.

In order to save computations, the support radius of the Gaussian cell aperture filters is defined to be $3 \sigmacellj$. This allows us to compute the histogram binnig aperture function once for each pixel in any cell and simply look up the value when needed in a cell. \Cref{fig:cellHistScaleSpacesSpatialWeights} shows
