...
When taking pictures of the real world, the resulting images are of various sizes and uses various zoom factors. This causes details to be present in the images at various scales. Using the Gaussian scale-space representation \cite{griffin1997scale,ginneken2000applications} described in this section, we are able to make a multi-scale representation of the details in an image. The representation concists of a set of smoothed versions of the original image, where each (scale-space) image is smoothed with a Gaussan kernel $G$ of size $\sigma$, which corresponds to the scale its smoothed image represents. For a point $(x,y)$, the Kernel at scale $\sigma$ is defined as
\begin{align}
	G(x,y;\sigma) &= \frac{1}{2\pi \sigma^2} \exp \left(\frac{-(x^2+y^2)}{2\sigma^2} \right)
\end{align}
and the corresponding scale-space point $L(x,y;\sigma)$ is defined as
\begin{align}
	L(x,y;\sigma) &= G(x,y;\sigma) \ast I(x,y),\hspace{1cm}\sigma \geq 0
\end{align}
where $I(x,y)$ is the intensity of the original image $I$ in $(x,y)$ and $\ast$ denotes the convolution operation defined as
\begin{align*}
	f(x,y) \ast g(x,y) &= \int\limits_{\tau_1 = -\infty}^\infty \int\limits_{\tau_2 = -\infty}^\infty f(\tau_1,\tau_2) \cdot g(x-\tau_1,y-\tau_2) d\tau_1 d\tau_2
\end{align*}
for the two functions $f$ and $g$.\todo{Add reference to Gonzales and woods}.

These scale space functions are defined on an infinite continuum but are in practice used on a finite set of scales, for which we wish to retrieve structural information in an image (\cite{lowe2004distinctive,pedersen2013shape,mikolajczyk2005performance} etc.).

\Cref{fig:scaleSpaceExample} shows an example of a finite scale-space representation of the image in \Cref{fig:scaleSpaceExample0} using the five scales $\sigma = 1,2,4,8,$ and $16$. At smaller scales (original, $\sigma = 1,2$) we are able to see detailed structure in the gates, pavement, car, and hedge etc. When increasing the scale to $\sigma = 4$, the small details are smoothed out and only the larger details of the mentioned physical parts are noticeable. At $\sigma = 8,$ and $16$ the details are completely smoothed out and only the larger overall structures are visible. This example clearly shows the power of the scale-space representation being able to model both small details as well as larger structures.
...
\begin{figure}
	...
	\end{subfigure}
	\caption{Example of a finite scale-space representation (5 scales) of an image from the INRIA dataset (\Cref{sec:odDataset}).}
...
The Gaussian derivatives $G_{x^m y^n}$, where $m$ is the order of derivations in the $x$-axis and $n$ is the order of derivations in the $y$-axis, are used for obtaining structural information from an image.
...
\begin{figure}
	\centering
	\includegraphics[width=\textwidth,clip=true,trim=31 0 0 0]{img/gradientOrientationTheory.pdf}
	\caption{Illustration of the gradient orientation}
	\label{gradientOrientationTheory}
\end{figure}

\begin{align}
\Theta &= \text{atan2}(L_y,L_x) \\
M &= \sigma^2 \sqrt{L_x^2 + L_y^2}
\end{align}

\subsection{Shape index and curvedness}
\label{sec:shapeIndexTheory}
%
The \emph{shape index} $S \in [-1,1]$ \cite{koenderink1992surface} is a scalar characterization of local second order derivatives. Like $\Theta$ and $M$, $S$ is independent of scale which is given by the \emph{curvedness} $C$. Additionally the shape index is independent of orientation.
...
\end{align}
%
\Cref{fig:shapeIndexTheory} illustrates the relationship between the principal curvatures, shape index and curvedness, as well as the geometry of various shape indices.
%
\begin{figure}
	\centering
	\includegraphics[width=\textwidth]{img/shapeIndexTheory.pdf}
