...
In theory the Gaussian function has infinite support causing the Gaussian filter to be unfeasibly large. In practice one often uses the 3-sigma rule to determine the support radius of the Gaussian filter and hence the computational load of the convolution. The rule states that approximately 99.7\% of the area under a 1D Gaussian function lies within 3$\sigma$ of the center of the function, and hence using $3\sigma$ as the Gaussian function support radius suffices.\todo{Determine if this should be stated here}
...
\end{align*}
%
Which give us the final equations for the alternative way of computing the Gaussian derivative scale-spaces. \todo{re-write}
...
%
\Cref{fig:gradientOrientationTheory} illustrates the relationship between the gradient orientation $\Theta$, gradient magnitude $M$, and $x$- and $y$-derivatives.
%
\begin{figure}[p]
	\centering
...
\end{align}
%
\todo{Should we avoid page breaks like this?}
...
%
We consider three choices of kernel functions for $A$ and $B$: box, triangle, and Gaussian. In 1D, the kernels are defined as\todo{Should we write $Box$, $Tri$ and $G$? after the names}
%
\begin{align*}
\mathit{Box} (x; \beta) &= 
\begin{cases}
...
\end{align*}
%
where $\x = [x ~~ y]^T$. We do not allow for separate scales $\alpha$ for each dimension. \todo{explain?}
%
\subsection{Periodic histograms}
\todo{introduce domains differently?}
...
When describing a local region in real world images, we experience a lot of transformations of the actual physical objects. We are however interested in being able to describe the region without any (or a predefined set) of these transformations applied, such that we get similar descriptions of the same region from images with different transformations. \todo{Maybe include more specific motivation?}
...
In this section we will go through a list of transformations that we wish for our descriptor to be invariant and possibly also robust to. \todo{explain better, move table}
...
The choice between using the ROC- or PR-curves and their AUCs are discussed in the individual application sections in \Cref{sec:od,sec:ic}.
%
